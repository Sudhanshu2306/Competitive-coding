...
void solve() {
    ...
    //         pn; return;
    //     }
    //     if(i<n-1 && j<m-1 && p[i]==s[j] && p[i+1]!=p[i] && s[j]==s[j+1]){
    //         i++; j+=2;
    //     }
    //     else{
    //         i++; j++;
    ...
    vector<pair<char,int>> pz=f(p);
    vector<pair<char,int>> sz=f(s);

    if(pz.size()!=sz.size()){pn; return;}

    for0(i,pz.size()){
        if(pz[i].f!=sz[i].f){flag=false; break;}
        ll k=pz[i].s,m=sz[i].s;
        if(m<k || m>2*k){flag=false; break;}
    }
    if(flag) py;
    else pn;
}
