...
*/
bool ff(ll x, ll y, ll z){
    if(x+y>z && x+z>y && z+y>x) return true;
    else return false;
}

bool fff(ll x) {return x!=0 && (x&(x-1))==0;}
...
void solve() {
    ...
    ll z1=x^y1,z2=x^y2;
    // if(ff(x,y1,z1)){cout<<y1<<endl; return;}
    // else if(ff(x,y2,z2)){cout<<y2<<endl; return;}
    // else cout<<-1<<endl;

    if(fff(x) || fff(x+1LL)){cout<<-1<<endl; return;}
    ll k=63-__builtin_clzll(x);
    ll q=__builtin_ctzll(x);
    ll p=-1;
    for0(i,k){
        if(((x>>i)&1LL)==0){
            p=i; break;
