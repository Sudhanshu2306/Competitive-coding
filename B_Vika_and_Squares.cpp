...
void solve() {
    ...
    vll pos;
    for0(i,n){
        if(a[i] == mini) pos.push_back(i);
    }

    int last_pos = pos.back() + (n - pos.back() + pos[0]);
    pos.push_back(last_pos);
 
    int maxN = 0, maxP1 = 0, maxP2 = 0;
    
    for0(i,pos.size()-1) {
        int g = abs(pos[i] - pos[i+1]);
        if (g > maxN) {
            maxP1 = pos[i];  
            maxP2 = pos[i+1];
            maxN = g;
    ...
    // Correctly print the result, adjust indexing if necessary
    cout<< (long long)a[maxP1] * n + abs(maxP1 - maxP2)-1 <<endl;
}
