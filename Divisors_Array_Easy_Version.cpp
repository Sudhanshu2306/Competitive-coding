...
unordered_map<int, int> ff(int m, vll& primes) {
...
unordered_map<int, int> fdf(int x, vll& primes) {
...
ll ffd(unordered_map<int, int>& factors) {
    ll x=1;
...
void solve() {
    ...
    unordered_map<int, int> mp=ff(m, primes);
    vll ans(n);
    for0(i,n){
        unordered_map<int, int> aFactors = fdf(a[i], primes);

        for(auto [prime, exponent] : mp){
            aFactors[prime]+=exponent;
        }
        ans[i] = ffd(aFactors);
    }
