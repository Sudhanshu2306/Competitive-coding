...
void solve() {
    // Your code goes here
    inll(n);
    instr(s); instr(p); // p is 26 len long;
    map<char,int> mp;
    for0(i,26) mp[p[i]]=i+1;

    map<char,char> mps; // rev mapping
    for0(i,26){
        int x=(27-mp['a'+i]);
        if(x>=1 && x<=26) mps['a'+i]=p[x-1];
        else mps['a'+i]='a'+i;
    }
    int ans1=0,ans2=1;
    char original_prev=s[0];
    char new_prev=mps[s[0]];

    bool flag=true;
    for1(i,n-1){
        char curr=s[i];
        char new_curr=mps[s[i]];git

        int org_cost=1e9,new_cost=1e9;
        if(curr>=original_prev) org_cost=min(org_cost,ans1);
        if(curr>=new_prev) org_cost=min(org_cost,ans2);

        if(new_curr>=original_prev) new_cost=min(new_cost,ans1+1);
        if(new_curr>=new_prev) new_cost=min(new_cost,ans2+1);

        if(org_cost==1e9 && new_cost==1e9) {flag=false; break;}

        original_prev=curr; new_prev=new_curr;
        ans1=org_cost; ans2=new_cost;
    }
    if(flag) cout<<min(ans1,ans2)<<endl;
    else cout<<-1<<endl;
