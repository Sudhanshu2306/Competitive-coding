...
bool isSquare(int l, int r, int d, int u){
    ...
    set<ll> st;
    st.insert(ff(L.f, L.s, R.f, R.s)); 
    st.insert(ff(L.f, L.s, D.f, D.s)); 
    st.insert(ff(L.f, L.s, U.f, U.s)); 
    st.insert(ff(R.f, R.s, D.f, D.s)); 
    st.insert(ff(R.f, R.s, U.f, U.s)); 
    st.insert(ff(D.f, D.s, U.f, U.s)); 
    return (st.size() == 2);
}

void solve() {
    // Your code goes here
    inll(l); inll(r); inll(d); inll(u);
    if(l==r && r==d && d==u) py;
    else pn;

    // if(isSquare(l,r,d,u)) py;
    // else pn;
}
