...
void solve() {
    ...
    while(q--){
        ...
        if(x=="->"){
            inll(r); inll(c);
            auto fun=[&](auto &&f, ll x, ll y, ll sz, ll val)-> ll {
                if(sz==1) return val;
                ll tot=sz*sz;
                ll subtot=tot/4;
                ll nsz=sz/2;
                ...
                for0(i,4){
                    if(r>=x+dx[i] && r<x+dx[i]+nsz && c>=y+dy[i] && c<y+dy[i]+nsz){
                        ll nval=val+subtot*i;
                        return f(f,x+dx[i],y+dy[i],nsz,nval);
                    }
                }
            };
            cout<<fun(fun,1,1,1<<n,1)<<endl;
        }
        else{
            inll(d);
            auto fun=[&](auto &&f, ll x, ll y, ll sz, ll val)-> pair<ll,ll> {
                if(d==val) return {x,y};
                ll tot=sz*sz;
                ll subtot=tot/4;
                ll nsz=sz/2;
                ...
                for0(i,4){
                    if(d>=val+subtot*i && d<val+subtot*(i+1)){
                        ll nval=val+subtot*i;
                        if(i==0) return f(f,x,y,nsz,nval);
                        if(i==1) return f(f,x+nsz,y+nsz,nsz,nval);
                        if(i==2) return f(f,x+nsz,y,nsz,nval);
                        if(i==3) return f(f,x,y+nsz,nsz,nval);
                    }
                }
                return {-1,-1};
            };
            auto ans=fun(fun,1,1,1<<n,1);
