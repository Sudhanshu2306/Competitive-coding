...
ll ff(ll i, ll n, ll m, vll b, vll pre){
    if(i==n) return 0;
    ...
    for1(k,m){
        ll x=b[k-1];
        ll j=upper_bound(all(pre),pre[i]+x)-pre.begin()-1;
        if(j<=n && j>=i) ans=min(ans,ff(j+1,n,m,b,pre)+(ll)(m-k));
    }
    return dp[i]=ans;
}
...
void solve() {
    ...
    // ll ans=ff(0,n,m,b,pre);
    for1(i,m){
        ll x=b[i-1];
        for0(j,n){
            if(dp[j]==1e18) continue;
            ll k=upper_bound(all(pre),pre[j]+x)-pre.begin()-1;
            if(k>=j+1 && k<=n) dp[k]=min(dp[k],dp[j]+(ll)(m-i));
        }
    }
