...
void solve() {
    // Your code goes here
    inll(n); vector<pll> a(n);
    multiset<ll> x,y;
    for0(i,n){
        cin>>a[i].f>>a[i].s;
        x.insert(a[i].f); y.insert(a[i].s);
    } 

    if(n==1){cout<<1<<endl; return;}
    ll ans=1e18;
    for(auto it:a){
        x.extract(it.f); y.extract(it.s);
        // ans=min(ans,f(row,col,n-1));
        ll l=*x.begin(),r=*x.rbegin();
        ll b=*y.begin(),t=*y.rbegin();
        ll w=(r-l+1), h=(t-b+1);
        ll area=w*h;
        if(area==n-1) ans=min(ans,area+min(w,h));
        else ans=min(ans,area);
        x.insert(it.f); y.insert(it.s);
    }
    cout<<ans<<endl;
}
