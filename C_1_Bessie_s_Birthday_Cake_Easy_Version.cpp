...
void solve() {
    // Your code goes here
    inll(n); inll(x); inll(y); //y=0
    vll a(x);
    for0(i,x) cin>>a[i];
    sort(a);

    ll ans=x-2;
    // a[i]=curr, a[i+1]=next & a[i+2]=after-next
    // so next is there or not no effect, coz it will form an edge with prev and if
    // consider making diadonal out of it it will be intersecting
    
    for0(i,x){
        if((a[i]-a[(i+1)%x]+2)%n==0) ans++;
    }
    cout<<ans<<endl;
