...
void solve() {
    ...
    for0(i,16){
        ...
        if(i==0) ans[x[i].second]=4;
        if(i>=1 && i<=8) ans[x[i].second]=3;
        if(i>=9 && i<=12) ans[x[i].second]=2;
        if(i>=13 && i<=14) ans[x[i].second]=1;
        if(i==15) ans[x[i].second]=0;
    }
    for0(i,16) cout<<ans[i]<<" ";
    cout<<endl;
