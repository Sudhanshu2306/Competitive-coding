...
void solve() {
    ...
    while(x>1 && k){
        int add=y-x%y; // this many to add before division;
        if(add<k){
            k-=add; // directly reduce k
            x+=add;
        }
        // edge case
        else{ // no point in adding more if k is less
            x+=k;
            k=0;
        }
        while(x%y==0) x/=y;
    ...
    if(k){
        k%=(y-1); // repeating steps after each (y-1) moves
        x+=k;
