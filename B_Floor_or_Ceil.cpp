...
/*Common things to remember : 
    ...
    15. BIT manupulation mein XOR, AND, OR, given question kogit binary (0/1) form mein socho, jab kuch dimag mein nahi aa rha, pakka bits se banega
*/

void solve() {
    // Your code goes here
    inll(x); inll(n); inll(m);
    ll minn=min(n,100ll);
    ll minm=min(m,100ll);
    ll curr=x;
    for0(i,minm) curr=(curr+1)/2;
    for0(i,minn) curr/=2;
    cout<<curr<<" ";
