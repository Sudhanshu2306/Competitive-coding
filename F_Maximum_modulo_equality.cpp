...
void build(const vll& arr, ll node, ll start, ll end) {
    ...
    else{ll mid=(start+end)/2; build(arr,2*node,start,mid); build(arr,2*node+1,mid+1,end); tree[node]=__gcd(tree[2*node],tree[2*node+1]);}
}
...
void solve() {
    ...
    vll diff(n-1);
    for0(i,n-1) diff[i]=abs(a[i+1]-a[i]);
    ...
    while(q--){
        inll(l); inll(r);
        l--; r--;
        ll gcdq=queryRange(1,0,n-2,l,r-1);
        if(gcdq==0) cout<<0<<" ";
        else cout<<gcdq<<" ";
    }
    cout<<endl;
