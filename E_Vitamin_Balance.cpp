...
bool f(ll mid, vvll &dp, ll x){
    ll count=0;
    for1(i,3){
        int y=lower_bound(all(dp[i]),mid)-dp[i].begin();
        count+=y;
    }
    return count<=x;
}
...
void solve() {
    ...
    // dp state will be paticular vitamin ke liye aur particular calories ke liye kitni units hongi
    // dp[vitamin_type][calorie_count]=no_of_units;

    vvll dp(3+1,vll(x+1,0));

    for0(i,n){
        for(ll c=x;c>=a[i][2];c--){
            dp[a[i][0]][c]=max(dp[a[i][0]][c],dp[a[i][0]][c-a[i][2]]+a[i][1]);
        }
    }
