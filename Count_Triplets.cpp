...
void solve() {
    ...
    vll a(n);
    for0(i,n) cin>>a[i];
    // abs(a[i]-a[k]) should be equal to abs(i-j)+abs(j-k)
    // abs(i-j) + abs(j-k) = (opening modulus)
    // (i) i-k, (ii) i+k-2*j, (iii) 2*j-i-k, (iv) k-i 
    int count=0;
    for0(i,n){
        for(int k=max(0,i-100);k<min(n,i+100);k++){
            int diff=abs(a[i]-a[k]);
            if(diff==abs(i-k)) count+=abs(i-k)+1;
            if((i+k+diff)%2==0){
                int j=(i+k+diff)/2;
                if(j<n && j>max(i,k)) count++;
            }
            if((i+k-diff)%2==0){
                int j=(i+k-diff)/2;
                if(j>=0 && j<min(i,k)) count++;
            }
        }
    }
    cout<<count<<endl;
