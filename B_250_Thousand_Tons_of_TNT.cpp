...
int maximizeAbsoluteDifference(vector<ll>& arr, int k) {
    int n = arr.size();

    sort(arr.begin(), arr.end());

    int groupSize = n / k;
    ll maxAbsoluteDiff = 0;
    ...
    for (int i = 0; i < k; ++i) {
        ...
        maxAbsoluteDiff = max(maxAbsoluteDiff, abs(sum - arr[startIdx]));
    }
...
void solve() {
    ...
    for(int i=1;i<n;i++){
        if(n%i==0){
            int x = maximizeAbsoluteDifference(arr,i);
            if(x>maxi){
                maxi=x;
            }
        }
