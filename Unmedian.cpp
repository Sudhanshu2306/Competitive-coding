...
const int ddd=100000; // Adjust based on problem constraints
ll tree[4*ddd], lazy[4*ddd];
...
void solve() {
    ...
    int flag=0;
    while(1){
        vll temp=a; sort(temp);
        if(a==temp){
            flag=1; break;
        }
        ll ind=-1,ans;
        if(a.size()<=2) break; x.push_back({1,3}); 
        vll temp1={a[0], a[1], a[2]};
        vll temp2=temp1; sort(temp2);
        ans=temp2[1];
        for0(i,3){
            if(temp1[i]==ans){
                ind=i; break;
        ...
        if(ind!=-1) a.erase(a.begin()+ind);
    }
    if(flag==0) {
        cout<<-1<<endl; return;
    }
    cout<<x.size()<<endl;
    for0(i,x.size()) cout<<x[i].f<<" "<<x[i].s<<endl;
