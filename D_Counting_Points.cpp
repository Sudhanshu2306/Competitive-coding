...
void solve() {
    ...
    inll(n); inll(m);
    vll x(n),r(n);
    ll xmini=1e18, xmaxi=-1e18; //left aur right limit
    ...
    for0(i,n){
        ll left=x[i]-r[i];
        ll right=x[i]+r[i];
        xmini=min(xmini,left);
        xmaxi=max(xmaxi,right);
        fori(j,left,right){
            ll dx=j-x[i];
            ll d=r[i]*r[i]-dx*dx;
            if(d<0) continue;
            ll maxiy=(ll)floor(sqrtl(d));
            mp[j].push_back({-maxiy, maxiy}); //x coord as the key
        }
    }
    
    ll count=0;
    // TLE de raha, obviously dega
    // fori(i,xmini,xmaxi){
    //     fori(j,-maxir,maxir){
    //         bool flag=false;
    //         for0(k,n){
    //             ll nx=i-x[k], ny=j;  
    //             // naye circle ka radius sabko cover kar raha h ya nahi
    //             ll d=nx*nx+ny*ny;
    //             if(d<=r[k]*r[k]){
    //                 flag=true; break;
    //             }
    ...
    for(auto &it:mp){
        vector<pll> &z=it.s; sort(z);
        ll start=z[0].f, end=z[0].s;
