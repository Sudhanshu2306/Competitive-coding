...
// most used function
ll binpow(ll a,ll b){if(b==0){return 1;}if(b%2){return (a*binpow(a,b-1))%M;}return binpow((a*a)%M,b/2);}
...
void solve() {
    ...
    sum=sum*sum; sum%M;
    sum=(sum-s+M)%M;
    sum=(sum*binpow(2,M-2))%M;
    ll ans=n*(n-1)/2; ans%=M;
    cout<<(sum%M)*binpow(ans,M-2)%M<<endl;
}
