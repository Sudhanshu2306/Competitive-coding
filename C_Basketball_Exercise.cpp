...
ll ff(vll &h1, vll &h2, ll i, ll prev, vvll &dp){
    if(i>=h1.size()) return 0;

    if(dp[i][prev]!=-1) return dp[i][prev];

    ll ans=0;
    ll nottake=max(ans,ff(h1,h2,i+1,prev,dp));
    ll take=-1e9;
    if(prev==0){
        take=max(take,ff(h1,h2,i+1,1,dp)+h1[i]);
        take=max(take,ff(h1,h2,i+1,2,dp)+h2[i]);
    }
    else if(prev==1) take=max(take,ff(h1,h2,i+1,2,dp)+h2[i]);
    else take=max(take,ff(h1,h2,i+1,1,dp)+h1[i]);
...
void solve() {
    ...
    for0(i,n) cin>>h1[i];
    for0(i,n) cin>>h2[i];
    // ll sum=SUM(h1)+SUM(h2);
    vvll dp(n+1,vll(4,-1));

    ll ans=ff(h1,h2,0,0,dp);
    cout<<ans<<endl;
}
