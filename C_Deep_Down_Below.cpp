...
bool ss(vector<pair<ll,vll>> &a, int mid){
    int n=a.size();
    for0(i,n){
        if(mid<a[i].f){
            return 0;
...
void solve() {
    // Your code goes here
    inint(n);
    vector<pair<ll,vll>> a(n);
    // ll mini=1e9, maxi=-1e9;
    for0(i,n){
        inint(k);
        vll x(k); for0(j,k) cin>>x[j];
        ll y=0;
        for0(j,k){
            y=max(y,x[j]-j+1);
        }
    ...
    // monotonic answer space, hmm.. binary-search!
    // agar x power ke liye condition stisfied h toh x+1,x+2, x+3 ke liye bhi hoga
