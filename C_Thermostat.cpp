...
void solve() {
    ...
    inll(a); inll(b);
    ll maxi=max(a,b); ll mini=min(a,b);
    if(mini<l || maxi>r){cout<<-1<<endl; return;}

    ll diff=maxi-mini;
    if(maxi==mini){cout<<0<<endl; return;}
    else if(x<=diff){cout<<1<<endl; return;}
    else if(x+maxi<=r || mini-x>=l){cout<<2<<endl; return;}
    else if(maxi-x>=l && mini+x<=r){cout<<3<<endl; return;}
    cout<<-1<<endl;
