...
int ff(int i, int j, string &a, string &b, string &c, vvll &dp){
    if(i==a.size() && j==b.size()) return 0;
    if(dp[i][j]!=-1) return dp[i][j];

    int ans=1e9;
    if(i<a.size()) ans=min(ans,ff(i+1,j,a,b,c,dp)+(a[i]!=c[i+j]));
    if(j<b.size()) ans=min(ans,ff(i,j+1,a,b,c,dp)+(b[j]!=c[i+j]));
