...
ll dfs(ll node, ll parent, vvll &adj, ll mid, ll &nc){
    ll size=1;
    for(auto it:adj[node]){
        if(it!=parent) size+=dfs(it,node,adj,mid,nc);
    }
    if(size>=mid){
        nc++; return 0;
    } 
...
void solve() {
    ...
    // bs on answer, toh agar saare components x size ke h atleast toh x-1,x-2 ke toh honge hi
