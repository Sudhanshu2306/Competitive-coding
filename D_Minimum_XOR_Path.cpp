...
*/
ll ans=1LL<<61;
void dfs(ll node, ll val, ll vis, ll n, vector<vector<pll>> &a){
    if(node==n) {ans=min(ans,val); return;}
    for(auto it:a[node]){
        if(!(vis&(1<<(it.f-1)))) dfs(it.f,val^it.s,vis|(1<<(it.f-1)),n,a);
    }
}
